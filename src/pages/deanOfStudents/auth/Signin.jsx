...
const Signin = () => {
  ...
  const onSubmit = data => {
    ...
    .then(response => {
      if (response.status === 200) {
        ...
        localStorage.setItem('dosToken', response.data.user.token);
        localStorage.setItem('dosData', JSON.stringify(response.data.user));
        window.location.replace(`/dos/${response.data.user.fullName.split(" ").join("").toLowerCase()}`);
      }
    })
    .catch(error => {
  ...
  return (
    <HorizontallyFlexSpaceBetweenContainer style={{ justifyContent: 'center', alignItems: 'center', paddingTop: '20px' }}>
      <Helmet>
        <title>Dean of Students - Login</title>
        <meta name="description" content={`Login as a Dean of Students.`} /> 
      </Helmet>
      ...
      <AuthenticationFormContainer style={{ borderBottom: '6px solid cyan', gap: '30px', position: 'relative', boxShadow: 'rgba(0, 0, 0, 0.05) 0 6px 24px, rgba(0, 0, 0, 0.08) 0 5px 12px 1px' }}>
        ...
        <VerticallyFlexGapForm style={{ gap: '20px'}} onSubmit={handleSubmit(onSubmit)}>
          ...
          <HorizontallyFlexSpaceBetweenContainer>
            <Link style={{ color: 'blue', textDecoration: 'none', fontSize:'90%', }} to={'/dos/auth/signup'}>Create account</Link>
            <Link style={{ textDecoration: 'none', fontSize:'90%', color: 'gray' }} to={'/dos/auth/forgot-password'}>Forgot Password?</Link>
          </HorizontallyFlexSpaceBetweenContainer>
