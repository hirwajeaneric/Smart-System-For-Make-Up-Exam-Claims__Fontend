...
const ClaimDetails = () => {
  ...
  // Data fetching 
  useEffect(() => {
    var currentUser = JSON.parse(localStorage.getItem('dosData'));
    setUser(currentUser);
  ...
  const handleClaimUpdates = (e) => {
    ...
    if (signature === 'Signed') {
      dos.signature = 'Signed';
    } else if (signature === 'Rejected') {
      dos.signature = 'Rejected';
    }
    ...
    if (signature) {
      dos.dateOfSignature = new Date().toUTCString();
    }

    updatedClaim.deanOfStudentsSignature = dos;
