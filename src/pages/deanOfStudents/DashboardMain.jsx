...
import { getAccountantClaims, getDeanOfStudentsClaims } from "../../redux/features/claimSlice";
...
const DashboardMain = () => {
    ...
    useEffect(() => {
        let user = JSON.parse(localStorage.getItem('dosData')); 
        setUser(user);
        dispatch(getAllCourses());
        dispatch(getDeanOfStudentsClaims({ token: user.token }));
    },[dispatch]);      

    const signout = () => {
        localStorage.removeItem('dosToken');
        localStorage.removeItem('dosData');
        navigate('/dosountant/auth/signin');
    }
    ...
    return (
        <VerticallyFlexSpaceBetweenContainer>
            <TopNavigationBar>
                <Link to={`/dos/${params.name}/`}><img src="/ssmec-logo-2.2.png" alt="" /></Link>  
                <div className="right">
                    ...
                    <Tooltip title="Account settings">
                        ...
                        >
                            <Avatar sx={{ width: 32, height: 32 }}>{getSimpleCapitalizedChars(JSON.parse(localStorage.getItem('dosData')).fullName)}</Avatar>
                        </IconButton>
                    </Tooltip>
                </div>
                <Menu
                ...
                >
                    ...
                    <Divider />
                    <MenuItem onClick={() => {navigate(`/dos/${user.departmentLink}/settings`); handleClose();}}>
                        <ListItemIcon>
