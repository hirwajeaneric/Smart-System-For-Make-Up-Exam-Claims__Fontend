...
const ClaimDetails = () => {
  ...
  // Data fetching 
  useEffect(() => {
    var currentUser = JSON.parse(localStorage.getItem('regData'));
    setUser(currentUser);
  ...
  const handleClaimUpdates = (e) => {
    ...
    if (comment) {
      examinationOfficer.comment = comment;
    }

    if (signature) {
      examinationOfficer.dateOfSignature = new Date().toUTCString();
    }
  ...
  return (
    <VerticallyFlexGapContainer style={{ gap: '20px' }}>
      <Helmet>
        <title>Examination Officer - Claim details</title>
