...
const DashboardMain = () => {
    ...
    useEffect(() => {
        let user = JSON.parse(localStorage.getItem('exoData')); 
        setUser(user);
    ...
    const signout = () => {
        localStorage.removeItem('exoToken');
        localStorage.removeItem('exoData');
        navigate('/examinationoffice/auth/signin');
    }
    ...
    return (
        <VerticallyFlexSpaceBetweenContainer>
            <TopNavigationBar>
                <Link to={`/examinationoffice/`}><img src="/ssmec-logo-2.2.png" alt="" /></Link>  
                <div className="right">
                    ...
                    <Tooltip title="Account settings">
                        ...
                        >
                            <Avatar sx={{ width: 32, height: 32 }}>{getSimpleCapitalizedChars(JSON.parse(localStorage.getItem('regData')).fullName)}</Avatar>
                        </IconButton>
                    </Tooltip>
                </div>
                <Menu
