...
const Signin = () => {
  ...
  const onSubmit = data => {
    ...
    .then(response => {
      if (response.status === 200) {
        ...
        localStorage.setItem('exoToken', response.data.user.token);
        localStorage.setItem('exoData', JSON.stringify(response.data.user));
        window.location.replace(`/examinationoffice/${response.data.user.fullName.split(" ").join("").toLowerCase()}`);
      }
    })
    .catch(error => {
  ...
  return (
    <HorizontallyFlexSpaceBetweenContainer style={{ justifyContent: 'center', alignItems: 'center', paddingTop: '20px' }}>
      <Helmet>
        <title>Examination Officer - Login</title>
        <meta name="description" content={`Login as a Examination Officer.`} /> 
      </Helmet>
