...
const Signup = () => {
  ...
  const onSubmit = data => {
    if (data.password !== data.confirmPassword) {
      ...
      data.role = 'Registration Officer';
      ...
      .then(response => {
        setTimeout(() => {
          ...
          if (response.status === 201) {
            window.location.replace(`/registration/auth/signin`);
          }
        }, 2000)
      })
      .catch(error => {
  ...
  return (
    <HorizontallyFlexSpaceBetweenContainer style={{ justifyContent: 'center', alignItems: 'center' }}>
      <Helmet>
        <title>Registration Officer - Create account</title>
        <meta name="description" content={`Create an account as a registration officer.`} /> 
      </Helmet>
      <AuthenticationFormContainer style={{ borderBottom: '6px solid cyan',gap: '30px', position: 'relative', boxShadow: 'rgba(0, 0, 0, 0.05) 0 6px 24px, rgba(0, 0, 0, 0.08) 0 5px 12px 1px' }}>
        ...
        <VerticallyFlexGapForm style={{ gap: '20px'}} onSubmit={handleSubmit(onSubmit)}>
          ...
          <HorizontallyFlexSpaceBetweenContainer>
            ...
            <Link style={{ color: 'blue', fontSize:'90%', textDecoration: 'none' }} to={'/registration/auth/signin'}>Sign in</Link>
          </HorizontallyFlexSpaceBetweenContainer>
