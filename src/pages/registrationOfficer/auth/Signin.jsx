...
const Signin = () => {
  ...
  const onSubmit = data => {
    ...
    .then(response => {
      if (response.status === 200) {
        ...
        localStorage.setItem('regToken', response.data.user.token);
        localStorage.setItem('regData', JSON.stringify(response.data.user));
        window.location.replace(`/registration/${response.data.user.fullName.split(" ").join("").toLowerCase()}`);
      }
    })
    .catch(error => {
  ...
  return (
    <HorizontallyFlexSpaceBetweenContainer style={{ justifyContent: 'center', alignItems: 'center', paddingTop: '20px' }}>
      <Helmet>
        <title>Registration Officer - Login</title>
        <meta name="description" content={`Login as a Registration Officer.`} /> 
      </Helmet>
      ...
      <AuthenticationFormContainer style={{ borderBottom: '6px solid cyan', gap: '30px', position: 'relative', boxShadow: 'rgba(0, 0, 0, 0.05) 0 6px 24px, rgba(0, 0, 0, 0.08) 0 5px 12px 1px' }}>
        <VerticallyFlexGapContainer style={{ gap: '10px' }}>
          ...
          <span style={{ color: 'black', fontWeight: '600' }}>Registration Officer</span>
        ...
        <VerticallyFlexGapForm style={{ gap: '20px'}} onSubmit={handleSubmit(onSubmit)}>
          ...
          <HorizontallyFlexSpaceBetweenContainer>
            <Link style={{ color: 'blue', textDecoration: 'none', fontSize:'90%', }} to={'/registration/auth/signup'}>Create account</Link>
            <Link style={{ textDecoration: 'none', fontSize:'90%', color: 'gray' }} to={'/registration/auth/forgot-password'}>Forgot Password?</Link>
          </HorizontallyFlexSpaceBetweenContainer>
