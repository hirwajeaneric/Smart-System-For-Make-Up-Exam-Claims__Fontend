  ...
  const onSubmit = data => {
    ...
    .then(response => {
      if (response.status === 200) {
        ...
        localStorage.setItem('accToken', response.data.user.token);
        localStorage.setItem('accData', JSON.stringify(response.data.user));
        window.location.replace(`/accountant/${response.data.user.fullName.split(" ").join("").toLowerCase()}`);
      }
    })
    .catch(error => {
  ...
  return (
    <HorizontallyFlexSpaceBetweenContainer style={{ justifyContent: 'center', alignItems: 'center', paddingTop: '20px' }}>
      <Helmet>
        ...
        <meta name="description" content={`Login as a accountant.`} /> 
      </Helmet>
